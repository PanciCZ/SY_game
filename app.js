// SY Mr X v15 – tabs + thicker map edges + MrX cards (double uses two rows) + players table
...
document.getElementById('togglePaths').addEventListener('click',()=>{SHOW_PATHS=!SHOW_PATHS; draw();});
...
function draw(){
  ...
  if(SHOW_PATHS){
    ...
    for(const s of segs.slice(0,-1)){
      ...
      // outline (thicker): white for black segments else black
...
function renderPlayersTable(){
  ...
  for(let step=1; step<=24; step++){
    ...
    for(const id of DETECTIVE_IDS){
      ...
      inp.addEventListener('change',()=>{const v=parseInt(inp.value||'',10); playersState[id][step]=Number.isFinite(v)?v:null;});
