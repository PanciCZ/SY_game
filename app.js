// Mister X viewer (v5) – solid badges, robust pan/zoom
...
function norm(g){const n=(g.nodes||[]).map(x=>({id:x.id??x.node??x.name,x:+x.x,y:+x.y,label:String(x.label??x.id??'')}));const e=(g.edges||g.links||[]).map(x=>({from:x.from??x.source,to:x.to??x.target,type:String(x.type??x.transport??'').toLowerCase()}));return{nodes:n,edges:e};}
...
// draw
function draw(){const d=state.dpi;ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle='#0b0f14';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.save();ctx.scale(d,d);ctx.translate(state.panX,state.panY);ctx.scale(state.scale,state.scale);
  if(state.img)ctx.drawImage(state.img,0,0);
  if(state.showEdges){ctx.lineCap='round';for(const e of state.graph.edges){const a=state.graph.nodes.find(n=>String(n.id)===String(e.from));const b=state.graph.nodes.find(n=>String(n.id)===String(e.to));if(!a||!b)continue;const col=e.type==='taxi'?'#FFD54A':e.type==='bus'?'#4CD964':e.type==='metro'?'#FF3B30':'#9CA3AF';ctx.strokeStyle=col;ctx.lineWidth=state.edgeWidth;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}}
  if(state.showNodes||state.showLabels){ctx.font=`800 ${state.fontPx}px system-ui,-apple-system,Segoe UI,Roboto,Inter,Arial,sans-serif`;for(const n of state.graph.nodes){if(state.showNodes){ctx.beginPath();ctx.arc(n.x,n.y,state.nodeRadius,0,Math.PI*2);ctx.fillStyle='#FFFFFF';ctx.fill();ctx.lineWidth=3;ctx.strokeStyle='rgba(0,0,0,0.75)';ctx.stroke();}
    if(state.showLabels){const t=String(n.label??n.id),padX=8,padY=4,r=10,tw=ctx.measureText(t).width,th=state.fontPx,bx=n.x,by=n.y-state.nodeRadius-(th*0.6)-6,x0=bx-(tw/2)-padX,y0=by-(th/2)-padY,x1=bx+(tw/2)+padX,y1=by+(th/2)+padY;ctx.beginPath();ctx.moveTo(x0+r,y0);ctx.lineTo(x1-r,y0);ctx.quadraticCurveTo(x1,y0,x1,y0+r);ctx.lineTo(x1,y1-r);ctx.quadraticCurveTo(x1,y1,x1-r,y1);ctx.lineTo(x0+r,y1);ctx.quadraticCurveTo(x0,y1,x0,y1-r);ctx.lineTo(x0,y0+r);ctx.quadraticCurveTo(x0,y0,x0+r,y0);ctx.closePath();ctx.fillStyle='rgba(255,255,255,0.95)';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle='rgba(0,0,0,0.75)';ctx.stroke();ctx.fillStyle='#0b0f14';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(t,bx,by);}}}
  ctx.restore();}
...
function edgeExists(a,b,type){return state.graph.edges.some(e=>{const f=String(e.from),t=String(e.to),tt=String(e.type).toLowerCase();const aa=String(a),bb=String(b);return(((f===aa&&t===bb)||(f===bb&&t===aa))&&(tt===type||(type==='black'&&(tt==='taxi'||tt==='bus'||tt==='metro'||tt==='ferry'))));});}
document.getElementById('mrxCommit').onclick=()=>{const ticket=document.getElementById('mrxTicket').value;const to=parseInt(document.getElementById('mrxDest').value,10);const dbl=document.getElementById('mrxDouble').checked;if(Number.isNaN(to)){addLog('Zadej cílový uzel.','err');return;}const last=game.moves.length?game.moves[game.moves.length-1].to:null;if(last&&ticket!=='black'&&!edgeExists(last,to,ticket)){addLog(`Neplatná hrana ${last} → ${to} pro ${ticket}.`,'err');return;}if(ticket==='black'){if(RULES.black<=0){addLog('Došly Black.','err');return;}RULES.black--;}game.moves.push({ticket,to});if(dbl){if(RULES.double<=0){addLog('Došly Double.','err');return;}RULES.double--;const to2=parseInt(prompt('Druhý cíl:')||'',10);const t2=(prompt('Druhá jízdenka (taxi/bus/metro/black):','taxi')||'').toLowerCase();if(!to2||!['taxi','bus','metro','black'].includes(t2)){addLog('Neplatný druhý tah.','err');return;}if(t2!=='black'&&!edgeExists(to,to2,t2)){addLog('Neplatná druhá hrana.','err');return;}if(t2==='black'){if(RULES.black<=0){addLog('Došly Black pro 2. tah.','err');return;}RULES.black--;}game.moves.push({ticket:t2,to:to2});}const c=game.moves.length;if(RULES.reveal.includes(c)){addLog(`Odhalení po ${c}. tahu: <strong>${game.moves[c-1].to}</strong>`,'ok');}else{addLog(`Mr X ${ticket} → ?`,'ok');}refreshTickets();draw();};
