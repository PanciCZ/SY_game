...
function switchTab(to){CURRENT_TAB=to;tabs.forEach(t=>t.classList.toggle('active',t.dataset.tab===to));Object.entries(views).forEach(([k,el])=>el.classList.toggle('active',k===to));if(to==='map')fit();if(to==='mrx')renderMrXCards();if(to==='players')renderPlayersTable();}
...
canvas.addEventListener('pointermove',e=>{if(!pts.has(e.pointerId))return;const prev=pts.get(e.pointerId),cur={x:e.clientX,y:e.clientY};pts.set(e.pointerId,cur);const arr=[...pts.values()];if(arr.length===1){state.panX+=cur.x-prev.x;state.panY+=cur.y-prev.y;draw();return;}if(arr.length===2){const [a,b]=arr;const cx=(a.x+b.x)/2,cy=(a.y+b.y)/2;const dist=Math.hypot(b.x-a.x,b.y-a.y);if(!pinch){pinch={cx,cy,dist};return;}const by=dist/pinch.dist;zoomAt(cx,cy,by);state.panX+=(cx-pinch.cx);state.panY+=(cy-pinch.cy);pinch={cx,cy,dist};}});
canvas.addEventListener('dblclick',e=>{const p=screenToBoard(e.clientX,e.clientY);const n=nearestNode(p.x,p.y); if(n) focusOn(n); else zoomAt(e.clientX,e.clientY,1.4);});
...
const game={moves:[]}; const commitStack=[]; let ACCRUED={black:0,double:0};
...
function addLog(m,c=''){const d=document.createElement('div');d.className='entry '+c; d.innerHTML=m; const l=document.getElementById('log'); if(l) l.prepend(d);}
...
document.getElementById('mrxCommit').onclick=()=>{
  ...
  if(movesTurn.length===2){const [a,b]=movesTurn; addLog(`Mr X double: ${a.ticket} → ${a.to}, ${b.ticket} → ${b.to}`,'ok');}
  else { addLog(`Mr X ${ticket} → ${to}`,'ok'); }
  const c=game.moves.length; if(REVEAL_STEPS.includes(c)){ addLog(`Odhalení po ${c}. tahu: <strong>${game.moves[c-1].to}</strong>`,'ok'); }
...
function draw(){
  ...
  if(SHOW_X_PATHS){
    for(const s of segs.slice(0,-1)){
      ...
      ctx.lineCap='round'; ctx.setLineDash([]); ctx.strokeStyle=(s.type==='black')?'#FFFFFF':'#000000'; ctx.lineWidth=34-6; // 28 outline
      ctx.beginPath(); ctx.moveTo(ax,ay); ctx.lineTo(bx,by); ctx.stroke();
      ctx.strokeStyle=col; ctx.lineWidth=30-6; ctx.setLineDash(isDashed(s.type)?[12,8]:[]); // 24 color
...
function renderMrXCards(){
  ...
  list.innerHTML=''; let step=1;
  for(const turn of commitStack){ for(const m of turn.moves){ const li=document.createElement('li'); li.className='mrx-card'; const left=document.createElement('div'); left.innerHTML=`<span class="mrx-step">${step}.</span> <span class="mrx-badge ${m.ticket}">${m.ticket}</span>`; const right=document.createElement('div'); if(REVEAL_STEPS.includes(step)) right.innerHTML=`<span class="mrx-reveal">${m.to}</span>`; li.appendChild(left); li.appendChild(right); list.appendChild(li); step++; } }
...
const DETECTIVE_IDS=['D1','D2','D3','D4','D5','D6']; if(!window.playersState){ window.playersState=Object.fromEntries(DETECTIVE_IDS.map(id=>[id, Array(25).fill(null)])); }
function renderPlayersTable(){
  ...
  trh.appendChild(document.createElement('th')).textContent='Tah'; for(const id of DETECTIVE_IDS){ const th=document.createElement('th'); th.textContent=id; trh.appendChild(th); }
  thead.appendChild(trh); table.appendChild(thead); const tbody=document.createElement('tbody');
  for(let step=1; step<=24; step++){ const tr=document.createElement('tr'); const th=document.createElement('th'); th.textContent=step; tr.appendChild(th);
    for(const id of DETECTIVE_IDS){ const td=document.createElement('td'); const inp=document.createElement('input'); inp.type='number'; inp.min='1'; inp.placeholder='?'; const cur=playersState[id][step]??''; if(cur!==null) inp.value=cur;
      inp.addEventListener('change',()=>{ const v=parseInt(inp.value||'',10); const val=Number.isFinite(v)?v:null; playersState[id][step]=val; const prev=playersState[id][step-1]; td.classList.remove('invalid'); if(prev && val){ if(!edgeExistsAny(prev,val)){ td.classList.add('invalid'); td.title='Neexistuje spojení mezi '+prev+' → '+val; } else { td.title=''; } } else { td.title=''; } draw(); });
